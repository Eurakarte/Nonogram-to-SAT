...
public NONO(Scanner in)
{
	...
	rowRuns = new int[rows][];
	columnRuns = new int[columns][];
	
	for (int i = 0; i < rowRuns.length; ++i)
	{
		rowRuns[i] = new int[in.nextInt()];
		for (int j = 0; j < rowRuns[i].length; ++j)
			rowRuns[i][j] = in.nextInt();
	}
	
	for (int i = 0; i < columnRuns.length; ++i)
	{
		columnRuns[i] = new int[in.nextInt()];
		for (int j = 0; j < columnRuns[i].length; ++j)
			columnRuns[i][j] = in.nextInt();
	}
}

public CNF constructSAT()
{
	int gridPositions = rowRuns.length * columnRuns.length;
	CNF SATProblem = new CNF();
	
	int auxVariableCount = 0;
	for (int i = 0; i < rowRuns.length; ++i)
	{
		int n = columnRuns.length; // n, number of entries in a row is the number of columns in the grid
		CNF constraint = new CNF(n, new NFA(rowRuns[i]));
		for (int j = 0; j < constraint.cl.size(); ++j)
		{
			for (int k = 0; k < constraint.cl.get(j).size(); ++k)
			{
				int variableIndex = constraint.cl.get(j).get(k);
				int parity = variableIndex < 0 ? -1 : 1;
				variableIndex = Math.abs(variableIndex);
				int offsetIndex;
				
				// the first case corresponds to an auxiliary variable
				if (variableIndex > n)
					variableIndex = gridPositions + auxVariableCount + (variableIndex - n);
				else
					variableIndex = (i * n) + variableIndex;
				
				constraint.cl.get(j).set(k, parity*variableIndex);
			}
		}
		
		auxVariableCount += constraint.V - n;
		SATProblem.C += constraint.C;
		SATProblem.V += constraint.V;
		SATProblem.cl.addAll(constraint.cl);
	}
	
	for (int i = 0; i < columnRuns.length; ++i)
	{
		int n = rowRuns.length;
		CNF constraint = new CNF(n, new NFA(columnRuns[i]));
		for (int j = 0; j < constraint.cl.size(); ++j)
		{
			for (int k = 0; k < constraint.cl.get(j).size(); ++k)
			{
				...
				{
					// the board variables are indexed sequentially down each row, so going down a column 
					// increases the variable index by the number of entries in a row each time
					variableIndex = ((variableIndex - 1) * columnRuns.length) + i + 1; 
				}
...
public String toString()
{
	...
	str += rowRuns.length + " ";
	str += columnRuns.length + "\r\n";
	
	for (int i = 0; i < rowRuns.length; ++i)
	{
		str += rowRuns[i].length + " ";
		for (int j = 0; j < rowRuns[i].length; ++j)
			str += rowRuns[i][j] + " ";
		str += "\r\n";
	}
...
public static void main ( String[] args )
{
	...
	System.out.print(nono.constructSAT());
}
